...
- Post the sticky PR comment with `curl` against the GitHub REST API instead of the `gh` CLI. Comments now work on any runner (self-hosted runners without `gh` previously skipped them silently) and on GitHub Enterprise Server (the action follows `GITHUB_API_URL`). The comment also fires on `pull_request_target` events, and the sticky lookup pages through up to 3,000 comments so a busy PR updates the existing comment instead of stacking a new one. Still best-effort: a failed comment warns and never fails the gate.
- Test the comment path offline: the stub API now mocks the GitHub issue-comment endpoints (list/create/update, paginated, with a permission-denied token), covering comment creation, sticky in-place updates, pagination beyond page 1, 403 handling, `pull_request_target`, and `comment-on-pr: false`.
